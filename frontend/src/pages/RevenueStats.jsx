...
const RevenueStats = () => {
    const [stats, setStats] = useState({ totalRevenue: 0, averageOrderValue: 0, totalOrders: 0, orders: [] });
    ...
    useEffect(() => {
        if (range === 'custom' && (!customDates.start || !customDates.end)) return;
        ...
        const loadStats = async () => {
            try {
                ...
                if (data.orders.length > 0) {
                    setSelectedOrder((current) => current || data.orders[0]);
                } else {
        ...
        <div style={{ backgroundColor: colors.bg, minHeight: '100vh', padding: '40px', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', backgroundColor: '#e2e8f0', padding: '5px', borderRadius: '15px', gap: '5px' }}>
                    ...
                    <div style={{ width: '1px', height: '25px', backgroundColor: '#cbd5e1', margin: '0 10px' }} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '30px' }}>
                <div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px', marginBottom: '30px' }}>
                        <StatCard icon={<TrendingUp color={colors.peach} />} label="Total Revenue" val={formatPrice(stats.totalRevenue)} bg={colors.forest} color="#fff" />
                        <StatCard icon={<PieChart color={colors.forest} />} label="Average Order" val={formatPrice(stats.averageOrderValue)} bg="#fff" color={colors.forest} />
                            ...
                            <tbody>
                                {stats.orders.map((order) => (
                                    <tr
                                        ...
                                        style={{ borderBottom: '1px solid #f8f8f8', cursor: 'pointer', backgroundColor: selectedOrder?._id === order._id ? '#f0fdf4' : 'transparent' }}
                                    >
                                        <td style={{ padding: '15px', fontWeight: 'bold' }}>#{order.orderId || order._id.slice(-6)}</td>
                                        <td>{order.user?.firstName}</td>
                                        <td style={{ fontWeight: 800 }}>{formatPrice(order.totalPrice)}</td>
                    ...
                    <div style={{ backgroundColor: colors.white, borderRadius: '30px', padding: '30px', boxShadow: '0 10px 30px rgba(0,0,0,0.03)' }}>
                        ...
                        {calc.cost && calc.sell && (
                            <div style={{ backgroundColor: colors.forest, color: '#fff', padding: '25px', borderRadius: '20px', textAlign: 'center' }}>
                                ...
                                <h2 style={{ margin: '10px 0', fontSize: '36px', color: colors.peach }}>{formatPrice(calc.sell - calc.cost)}</h2>
                                <div style={{ fontSize: '14px', backgroundColor: 'rgba(255,255,255,0.1)', padding: '5px', borderRadius: '10px' }}>
                                    Margin: {((((calc.sell - calc.cost) / calc.sell) * 100) || 0).toFixed(1)}%
                                </div>
                            </div>
                        )}
                    </div>
...
const StatCard = ({ icon, label, val, bg, color }) => (
    <div style={{ backgroundColor: bg, color, padding: '25px', borderRadius: '25px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)' }}>
        {icon}
